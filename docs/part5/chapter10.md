...
<PromptCompare title="给AI准备材料的方式" badLabel="原始堆砌" goodLabel="结构化整理" hint="材料越有结构，AI越少猜测，输出越可检查。">
  <template #bad>
...
**任务**：分析以下材料中关于用户满意度的关键发现。

**材料A**（官方报告，高可靠）：2024年平台运营报告摘要.md

**材料B**（用户反馈，待核实）：整理后的用户评价.csv

**输出要求**：用表格归纳关键发现，标注数据来源。
...
<ThinkBox
  question="找一份你最近提交给AI的材料（论文草稿、课程笔记等），用本章的标准审视一遍：格式是否AI友好？结构是否清晰？有没有冗余信息？尝试用Markdown重新组织一下，看看AI回答质量是否有变化。"
  btnText="查看整理步骤"
>

整理步骤：(1) 检查格式：截图转文字，PDF转Markdown；(2) 清理冗余：删掉与任务无关的内容；(3) 添加结构：用标题分区，用列表提点，用表格对比；(4) 标注来源：标明每份材料的可靠程度。
