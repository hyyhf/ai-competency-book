...
<PromptCompare title="要求AI输出的方式" badLabel="自然语言输出" goodLabel="结构化输出" hint="当你需要对AI输出做批量检查或与其他工具对接时，结构化输出是更好的选择。">
  <template #bad>
...
请用JSON格式输出对这五篇论文的分析，每篇包含以下字段：title（标题）、method（研究方法）、sample_size（样本量）、key_finding（主要发现）、limitation（局限性）。
...
[^1]: JSON.org. Introducing JSON. https://www.json.org/json-en.html

[^2]: 朱俊仪, 朱尚明. (2024). 利用检索增强生成技术开发本地知识库应用.《通信学报》, 45(Z2), 242-247. https://www.joconline.com.cn/zh/article/doi/10.11959/j.issn.1000-436x.2024227/
